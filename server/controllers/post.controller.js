...
export const likePost = async (req,res) => {
    try {
        ...
        const user = await User.findById(loggedInUserId).select('username profilePicture');
...
export const addComment = async (req,res) => {
    try {
        ...
        if (!text) return res.status(400).json({message:'Text reqiured',success:false});
...
export const deletePost = async (req,res) => {
    try {
        ...
        if(post.author.toString() != authorId) return res.status(403).json({message:'You cannot Delete Others Post'});
...
export const bookmarkPost = async(req,res) => {
    try {
        ...
        if(user.bookmarks.includes(post._id)){
            ...
            await user.save();
            return res.status(200).json({type:'saved', message:'Post added to bookmark', success:true});
        }
    } catch (error) {
       console.log(error) 
    }
